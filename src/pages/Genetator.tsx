...
const Generator = () => {
  ...
  const handleGenerate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if(!user) return toast('Please fill all the required fields')
...
toast.error(error?.response?.data?.message || error.message);
      ...
      <form onSubmit={handleGenerate} className="max-w-4xl mx-auto mb-40">
        ...
        <div className="flex gap-20 max-sm:flex-col items-start justify-between">
          ...
          <div className="flex flex-col w-full sm:max-w-60 gap-8 mt-8 mb-12">
            ...
            />
            {/* Note: I moved the second UploadZone inside the flex container correctly */}
            <UploadZone
              label="Model Image"
              file={modelImage}
  ...
  <PrimaryButton disabled={isGenerating} className="px-10 py-3 roun disabled:opacity-70 disabled:cursor-not-allowed">
    {isGenerating ? (
      <>
